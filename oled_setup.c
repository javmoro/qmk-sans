...
static void render_sans(int SANS_X,
	...
	static const char PROGMEM anNormal[3][ANIM_SIZE_SANS] = {
		{	// 'sans0', 32x32px
			...
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1c, 0x3e, 0x3b, 0x6a, 0x15, 0x7b, 0x7a, 0x01, 0xad, 0xbd,
			0xba, 0xbd, 0xad, 0x01, 0x7a, 0x6b, 0x15, 0x7a, 0x3b, 0x3e, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x26, 0x29, 0x31, 0x31, 0x31, 0x11, 0x08, 0x04,
			0x02, 0x04, 0x08, 0x11, 0x31, 0x31, 0x31, 0x29, 0x26, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
		},
		{
			// 'sans1' 32x22px
	...
	void animate_cat(void) { // logica de la animacion del gato
		if (current_wpm < WPM) { // si current_wpm menor a WPM
			...
			if (led_usb_state.caps_lock) { // si esta activado Caps Lock
				...
				oled_write_raw_P(idle[abs(1 - current_cat_frame)], ANIM_SIZE_CAT); // sans ladra

			} else {
				oled_write_raw_P(idle[abs(1 - current_cat_frame)], ANIM_SIZE_CAT);
			}
